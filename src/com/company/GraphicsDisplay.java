...
public class GraphicsDisplay extends JPanel {
    ...
    private boolean showMarkers = true;// Флаговые переменные, задающие правила отображения графика
    private double minX;
    private double maxX;//границы
    private double minY;
    private double maxY;
    ...
    public void showGraphics(Double[][] graphicsData) {
        this.graphicsData = graphicsData;// Сохраняем массив точек
    ...
    public void paintComponent (Graphics g) {
        ...
        minX = graphicsData[0][0];// определение минимального и максимального значения для координат X и Y, это необходимо для определения области пространства, подлежащей отображению, её верхний левый угол это (minX, maxY) - правый нижний это (maxX, minY)
        maxX = graphicsData[graphicsData.length - 1][0];
        minY = graphicsData[0][1];
        maxY = minY;
        // минимальное и максимальное значение функци
        ...
        if (showAxis) paintAxis(canvas);// отрисовываются оси координат
        paintGraphics(canvas);//график
        if (showMarkers) paintMarkers(canvas);//маркеры точек
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        canvas.setColor(Color.BLACK);//контур маркеров
        canvas.setPaint(Color.BLACK);//закрашивание
        for (Double[] point : graphicsData) {
            ...
            canvas.draw(line);
            canvas.draw(marker);
            canvas.setColor(Color.BLACK);//цвет точек
        }
    }

    protected void paintAxis(Graphics2D canvas) {
        canvas.setStroke(axisStroke);
        canvas.setColor(Color.BLACK);//оси
        canvas.setPaint(Color.BLACK);
        canvas.setFont(axisFont);//подписи к осям
        // рисуем оси
        FontRenderContext context = canvas.getFontRenderContext();
        if (minX <= 0.0 && maxX >= 0.0) {
            canvas.draw(new Line2D.Double(xyToPoint(0, maxY), xyToPoint(0, minY)));//линия, ось
            GeneralPath arrow = new GeneralPath();//стрелка оси у
            Point2D.Double lineEnd = xyToPoint(0, maxY);//нач точка ломанной
    ...
    protected Point2D.Double xyToPoint(double x, double y) {
        ...
        // Вычисляем смещение Y от самой верхней точки (maxY)
    ...
    protected Point2D.Double shiftPoint(Point2D.Double src, double deltaX, double deltaY) {
        ...
        dest.setLocation(src.getX() + deltaX, src.getY() + deltaY);// Задать еѐ координаты как координаты существующей точки +
        //заданные смещения
        return dest;
    }
}